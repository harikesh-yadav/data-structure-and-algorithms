...
func detectCycleInUndirectedGraph(g *graph) bool {
	if len(g.vertices) == 0 {
		return false
	}
...
func main() {
	...
	g.addEdge(0, &vertex{val: 3})

	g.addEdge(1, &vertex{val: 0})
	g.addEdge(1, &vertex{val: 2})

	g.addEdge(2, &vertex{val: 1})
	//g.addEdge(2, &vertex{val: 0})

	g.addEdge(3, &vertex{val: 0})
	g.addEdge(3, &vertex{val: 4})
	g.addEdge(4, &vertex{val: 3})

	g.printgraph()

	fmt.Println("IS Graph have Cycle:", detectCycleInUndirectedGraph(g))
}
