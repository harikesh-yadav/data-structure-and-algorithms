...
func eggDropProblemRecursiveMemozation(egg, floor int) int {
	...
	for k := 1; k <= floor; k++ {
		...
		var down, top int
		if matrix[egg-1][k-1] != -1 {
			down = matrix[egg-1][k-1]
		} else {
			down = eggDropProblemRecursiveMemozation(egg-1, k-1)
		}

		if matrix[egg][floor-k] != -1 {
			top = matrix[egg][floor-k]
		} else {
			top = eggDropProblemRecursiveMemozation(egg, floor-k)
		}

		// use maximum for worst case
		temp := 1 + maximum(down, top)
