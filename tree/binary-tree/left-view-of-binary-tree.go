...
func leftViewOnBinaryTree(n *node, hash *map[int]int, level int) {
	if n == nil {
		return
	}
	...
	leftViewOnBinaryTree(n.left, hash, level+1)
	leftViewOnBinaryTree(n.right, hash, level+1)
}
...
func main() {
	...
	initialLevel := 0
	leftViewOnBinaryTree(bt.root, &hashmap, initialLevel)
