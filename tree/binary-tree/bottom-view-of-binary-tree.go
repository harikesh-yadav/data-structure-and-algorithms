...
func bottomViewOnBinaryTree(n *node, hash map[int]int, hd int) map[int]int {
	if n == nil {
		return nil
	}
	...
	for len(queue) > 0 {
		front := queue[0]
		// In Bottom view we have to replace hd value with last  value of same hd
...
func main() {
	...
	bottomViewOnBinaryTree(bt.root, hashmap, initialLevel)
